class WTVAuthor {
    ...
    checkPageIntroSeen() {
        return this.wtvclient.getSessionData("subscriber_page_intro_seen")
            ? this.wtvclient.getSessionData("subscriber_page_intro_seen")
            : false;
    }

    setPageIntroSeen(seen) {
        this.wtvclient.setSessionData(
            "subscriber_page_intro_seen",
            seen ? true : false
        );
    }
    ...
    createPagestore() {
        if (this.pagestoreExists() === false) {
            try {
                if (!this.fs.existsSync(this.pagestore_dir))
                    this.fs.mkdirSync(this.pagestore_dir, {recursive: true});
                return true;
            } catch {
            }
        }
    ...
    createPage(style) {
        ...
        if (this.fs.existsSync(pagefileout)) {
            console.error(
                " * ERROR: Page already exists (should never happen). Page lost."
            );
            return false;
        }
    ...
    loadPage(pagenum) {
        ...
        var pagestorepath = this.pagestore_dir;
        var self = this;
        var page_data_raw = self.fs.readFileSync(pagestorepath + pagenum + ".page");
    ...
    setStyle(style, title, desc, state, docName) {
        ...
        this.debug(
            "setStyle",
            "this.wtvshared.makeSafeStringPath(style) (before load)",
            this.wtvshared.makeSafeStringPath(style)
        );
        console.log(style);
        var template_data_file = this.wtvshared.getTemplate(
            "wtv-author",
            "styles/" + this.wtvshared.makeSafeStringPath(style) + ".js",
            true
        );
    ...
    generateBlock(number, page, state) {
        ...
        switch (type) {
            case "text":
                ...
                if (thisblock.size) block += " size=" + thisblock.size + ">";
                else block += ">";

                if (thisblock.style) block += "<" + thisblock.style + ">";
...
<TD valign=top>`;
                    if (
                        (state == "editing" && thisblock.caption == null) ||
                        (state == "editing" && thisblock.caption.length == 0)
                    )
                        block +=
                            "<i>Choose this to change the image or add an optional caption.</i>";
                    else if (thisblock.caption != null) block += `${thisblock.caption}`;
                    block += `</td>
</tr>
</tbody></table>
</b></font><p></p>
<p>&nbsp;</p>`;
            ...
            case "clipart":
                if (state == "publishing") {
                    ...
                    else block += `<TABLE nocolor width=100%>`;
                    if (thisblock.title)
                        block += `<TR>
<TD align=center>
<font color=${this.headcol}>
...
<hr>
                ...
                if (this.headerimgR) {
                    block += `<td><img src="${this.headerimgR}" width="${this.headerimgRwidth}" height="${this.headerimgRheight}"> </td>`;
                }
            ...
            case "list":
                ...
                else block += `<table nocolor="" cellspacing="0" cellpadding="0">`;
...
<a href="${thisblock.items[i].url}">`;
                    if (
                        thisblock.items[i].name != "" &&
                        thisblock.items[i].name != undefined
                    )
                        block += `${thisblock.items[i].name}`;
                    else block += `${thisblock.items[i].url}`;
                    block += `</a>
</li>
</ul>
</td>
...
<TR>
<TD valign=top>`;
                if (thisblock.caption != null) block += `${thisblock.caption}`;
                block += `
</TD>
</TR>
</TABLE>
                ...
                this.fs.writeFile(
                    ...
                    number +
                    ".jpg",
                    new Buffer.from(thisblock.photo, "base64"),
                    (err) => {
                        if (err) {
                            console.error(err);
                        }
...
<TD valign=top>`;
    ...
    generatePage(state, pagenum, page) {
        ...
        if (page != 1) {
            for (
                let i = pagedata.pagebreaks[page - 2];
                i < (pagedata.pagebreaks[page - 1] || pagedata.blocks.length);
                i++
            ) {
...
<a href="http://webtv.zone">Powered by WebTV</a>
</i></font></center>
</td>
...
<a href="`;
                ...
                if (state == "previewing" || state == "publishing")
                    if (state == "publishing") html += `page${parseInt(page) + 1}.html`;
                    else
                        html += `wtv-author:/preview?docName=${pagenum}&pageNum=${
                            parseInt(page) + 1
                        }`;
                html += `">next page</a>
</b></font></td>
...
<hr>`;
                ...
                page != 1 &&
                pagedata.pagebreaks.length + 1 == parseInt(page)
            ) {
                html += `<tr>
<td colspan="3">
<hr>
...
<a href="`;
                if (state == "previewing" || state == "publishing")
                    if (state == "publishing")
                        if (parseInt(page) == 2) html += `index.html`;
                        else html += `page${parseInt(page) - 1}.html`;
                    else
                        html += `wtv-author:/preview?docName=${pagenum}&pageNum=${
                            parseInt(page) - 1
                        }`;
...
<hr>`;
    ...
    editMetadata(title, description, showtitle, pagenum) {
        ...
        if (showtitle == "true") var showtitle2 = false;
        else var showtitle2 = true;
    ...
    listPages() {
        ...
        files.map(function (v) {
            ...
            if (self.fs.existsSync(pagepath))
                page_data_raw = self.fs.readFileSync(pagepath);
            if (page_data_raw) {
                var page_data = JSON.parse(page_data_raw);
                self.pageArr[v.split(".")[0]] = page_data;
            }
        });
        return self.pageArr;
    }
    ...
    deleteBlock(pagenum, position) {
        ...
        this.editPage(pagedata, pagenum);
        if (block.type == "break") this.generateBreakList(pagenum);
        return true;
    }
    ...
    publishPage(pagenum, listpublicly) {
        ...
        for (let i = 1; i < pagedata.pagebreaks.length + 2; i++) {
            ...
            if (i == 1) var fileout = "index.html";
            else var fileout = "page" + i + ".html";
            this.fs.writeFile(
                destDir +
                this.wtvclient.session_store.subscriber_username +
                "/" +
        ...
        for (let i = 0; i < this.stylemedia.length; i++) {
            this.fs.mkdirSync(
                ...
                "/" +
                publishname +
                this.stylemedia[i].substr(0, this.stylemedia[i].lastIndexOf("/")),
                {recursive: true}
            );
            this.fs.copyFile(
                "ServiceVault/wtv-author" + this.stylemedia[i],
                destDir +
                this.wtvclient.session_store.subscriber_username +
                "/" +
                publishname +
...
<td height=14>
<tr><td height=10>`;
        loop: for (let i = 0; i < numofpages; i++) {
            pageid = keys[i];
            if (
                pagearray[pageid].published == true &&
                pagearray[pageid].inlist == true
            ) {
                html += `<tr><td>
<td width=5>
<td>
...
<tr>
<td width=12>
<td><font size=-1>${pagearray[pageid].description}</font>
</table>
<tr><td height=10>
<tr><td height=10>`;
            } else {
    ...
    deletePage(pagenum) {
        ...
        this.pagestore_dir = userstore_dir + store_dir;
        var pagestorepath = this.pagestore_dir;
        var page_file_out = pagenum + ".page";
    ...
    editTextBlock(pagenum, title, caption, size, style, position, oldposition) {
        ...
        if (oldposition != position) pagedata.blocks.move(oldposition, position);

        this.editPage(pagedata, pagenum);
        return true;
    }

    createPhotoBlock(pagenum, photo, type, position) {
        var pagedata = this.loadPage(pagenum);
        if (!pagedata) return false;
    ...
    editPhotoBlock(pagenum, oldposition, position, photo, type, title, caption) {
        ...
        if (title != null) blocks[oldposition].title = title;

        if (caption != null) blocks[oldposition].caption = caption;
    ...
    createLinkBlock(pagenum, title, listItems, linkItems, position) {
        ...
        var items = [];
        loop: for (let i = 0; i < linkItems.length; i++) {
            var url = linkItems[i];
