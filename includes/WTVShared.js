...
class WTVShared {
    ...
    getQueryString(query) {
        ...
        if (typeof query === "object") return query[Object.keys(query).length - 1];
        else return query;
    }
    ...
    returnAbsolutePath(check_path) {
        if (
            check_path.substring(0, 1) != this.path.sep &&
            check_path.substring(1, 1) != ":"
        ) {
    ...
    //this function can go suck my balls
    ...
    */
    ...
    isOldBuild(ssid_session) {
        return ssid_session.get("wtv-need-upgrade") ||
            ...
            parseInt(ssid_session.get("wtv-system-version")) < 3500
            ? true
            : false;
    }
    ...
    getUserConfig() {
        try {
            var user_config_filename = this.getAbsolutePath(
                "user_config.json",
            ...
            try {
                ...
                try {
                    ...
                    this.fs.writeFileSync(
                        ...
                        JSON.stringify(new_user_config, null, "\t")
                    );
                    return true;
                } catch (e) {
                    ...
                    if (post_text.length > 0) {
                        ...
                        for (let i = 0; i < post_text.length; i++) {
                            ...
                            if (qraw_split.length == 2) {
                                var k = qraw_split[0];
                                post_obj.query[k] = unescape(
                                    post_text[i].split("=")[1].replace(/\+/g, "%20")
                                );
                            }
                        }
                    }
    ...
    getAbsolutePath(path, directory = null) {
        if (directory) {
            if (path.indexOf(directory) == -1) {
                ...
                try {
                    if (this.fs.lstatSync(directory).isDirectory())
                        directory = directory + this.path.sep;
                } catch (e) {
                }
                path = directory + path;
            }
    ...
    doErrorPage(
        ...
        if (wtv_reset && !pc_mode) {
            ...
            headers +=
                "wtv-visit: wtv-1800:/preregister?scriptless-visit-reason=999\n";
    ...
    fixPathSlashes(path) {
        // fix slashes
        if (this.path.sep == "/" && path.indexOf("\\") != -1)
            path = path.replace(/\\/g, this.path.sep);
        else if (this.path.sep == "\\" && path.indexOf("/") != -1)
            path = path.replace(/\//g, this.path.sep);

        // remove double slashes
        while (path.indexOf(this.path.sep + this.path.sep) != -1)
            path = path.replace(this.path.sep + this.path.sep, this.path.sep);
    ...
    unpackCompressedB64(data) {
        var data_buf =
            typeof data === "object"
                ? Buffer.from(data.toString("ascii"), "base64")
                : Buffer.from(data, "base64");
        return this.zlib
            .inflateSync(data_buf, { finishFlush: this.zlib.Z_SYNC_FLUSH })
            .toString("ascii");
    }
    ...
    getDynamicConfig(whichJason) {
        const yourMother = this.fs.readFileSync(
            `./DynamicConfig/${whichJason}.json`,
            { encoding: "utf8", flag: "r" }
        );
        console.log(`DynamicConfig given for "${whichJason}".`)
        return (JSON.parse(yourMother));
    }
}
...
class clientShowAlert {
    ...
    getURL() {
        ...
        if (this.buttonlabel1)
            url += "buttonlabel1=" + encodeURIComponent(this.buttonlabel1) + "&";
        if (this.buttonaction1)
            url += "buttonaction1=" + encodeURIComponent(this.buttonaction1) + "&";
        if (this.buttonlabel2)
            url += "buttonlabel2=" + encodeURIComponent(this.buttonlabel2) + "&";
        if (this.buttonaction2)
            url += "buttonaction2=" + encodeURIComponent(this.buttonaction2) + "&";
