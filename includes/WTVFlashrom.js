class WTVFlashrom {
    ...
    getFlashromInfo(data, path) {
        ...
        if (flashrom_info.magic == flashrom_magic)
            flashrom_info.valid_flashrom = true;
        if (!flashrom_info.valid_flashrom)
            console.error(
                " * Warning! FlashROM File Magic (" +
                flashrom_info.magic +
                ") did not match expected magic (" +
                flashrom_magic +
                ")..."
            );
        ...
        flashrom_info.percent_complete = (
            ((flashrom_info.byte_progress + flashrom_info.part_total_size) /
                flashrom_info.total_parts_size) *
            100
        ).toFixed(1);
        ...
        if (flashrom_info.is_last_part) {
            ...
            if (
                ...
                console.log(
                    " # Flashrom Next Part Number :",
                    flashrom_info.next_part_number
                );
        }
        ...
        if (
            ...
            console.log(
                " # Sending",
                flashrom_info.is_last_part ? "Last Flashrom" : "Flashrom",
                "Part",
                flashrom_info.part_number,
                "- Bytes Sent:",
                flashrom_info.byte_progress + flashrom_info.part_total_size,
                "of",
                flashrom_info.total_parts_size,
        ...
        flashrom_info.message = new Buffer.from(
            part_header.toString("hex").substring(36 * 2, 68 * 2),
            "hex"
        )
            .toString("ascii")
            .replace(/[^0-9a-z\ \.\-]/gi, "");
        ...
        if (flashrom_info.is_last_part && this.bf0app_update) {
            ...
            flashrom_info.next_rompath = "wtv-flashrom:/lc2-download-done?";
        } else {
            flashrom_info.next_part_number = this.formatPartNum(
                flashrom_info.part_number + 1
            );
            flashrom_info.next_rompath = flashrom_info.rompath.replace(
                "part" + this.formatPartNum(flashrom_info.part_number),
                "part" + flashrom_info.next_part_number
            );
        }
    ...
    async sendToClient(data, request_path, callback) {
        ...
        if (flashrom_info.is_bootrom)
            headers += "Content-Type: binary/x-wtv-bootrom"; // maybe?
            ...
            const req = this.https.request(options, function (res) {
                ...
                res.on("end", function () {
                    ...
                    if (res.statusCode == 200) {
                        ...
                        var data = self.getFlashromInfo(
                            Buffer.from(data_hex, "hex"),
                            request_path
                        );
                    } else if (res.statusCode == 404) {
                        var errpage = self.wtvshared.doErrorPage(
                            404,
                            "The service could not find the requested ROM on zefie's server."
                        );
                        headers = errpage[0];
                        var data = errpage[1];
                    } else {
