class WTVGuide {
    ...
    generatePage(topic, subtopic, page = null) {
        ...
        switch (topic.toLowerCase()) {
            case "glossary":
                var template = this.wtvshared.getTemplate(
                    "wtv-guide",
                    "templates/glossary.js",
                    true
                );
                var glossary_datafile = this.wtvshared.getTemplate(
                    "wtv-guide",
                    "glossary.json",
                    true
                );
                ...
                if (glossary[subtopic.toUpperCase()]) {
                    if (page) {
                        ...
                        if (glossary[subtopic.toUpperCase()][page.toLowerCase()]) {
                            var word =
                                glossary[subtopic.toUpperCase()][page.toLowerCase()].word;
                            var definition =
                                glossary[subtopic.toUpperCase()][page.toLowerCase()].definition;
                            // replace <word>the word</word> with a nice convienent link
                            var search = "<word";
                            while (definition.indexOf(search) >= 0) {
                                var link_word_for_link,
                                    link_word_start_letter,
                                ...
                                if (definition.substr(start, 1) != ">") {
                                    ...
                                    // strip any quotes
                                    if (link_word_override.substr(0, 1).match(/[\"\']/))
                                        link_word_override = link_word_override.substring(1);
                                    if (
                                        link_word_override
                                            .substr(link_word_override.length - 1, 1)
                                            .match(/[\"\']/)
                                    )
                                        link_word_override = link_word_override.substr(
                                            0,
                                            link_word_override.length - 1
                                        );

                                    link_word_for_link = link_word_override
                                        .replace(/ /g, "")
                                        .replace(/\'/g, "")
                                        .replace(/\"/g, "")
                                        .toLowerCase();
                                    link_word_start_letter = link_word_for_link
                                        .substr(0, 1)
                                        .toUpperCase();
                                ...
                                if (!link_word_for_link)
                                    link_word_for_link = link_word
                                        .replace(/ /g, "")
                                        .replace(/\'/g, "")
                                        .replace(/\"/g, "")
                                        .toLowerCase();
                                if (!link_word_start_letter)
                                    link_word_start_letter = link_word.substr(0, 1).toUpperCase();
                                if (!link_word_override) link_word_override = link_word;

                                var link = `wtv-guide:/help?topic=Glossary&subtopic=${link_word_start_letter}&page=${link_word_for_link}&word=${encodeURIComponent(
                                    link_word_override
                                )}`;
                                var new_definition =
                                    definition.substring(0, original_start - search.length) +
                                    `<a href="${link}">${link_word}</a>` +
                                    definition.substring(end + 7);
                            ...
                            while (definition.indexOf("<boxname>") >= 0) {
                                ...
                                var boxname = "";
                                if (
                                    romtype == "US-WEBSTAR-disk-0MB-16MB-softmodem-CPU5230" ||
                                    romtype == "US-DTV-disk-0MB-32MB-softmodem-CPU5230"
                                )
                                    boxname = "satellite receiver";
                                else if (this.session_data.hasCap("client-has-tv-experience"))
                                    boxname = "WebTV Plus receiver";
                            ...
                            while (definition.indexOf("<boxname_plus>") >= 0) {
                                var boxname = "WebTV";
                                if (this.session_data.hasCap("client-has-tv-experience"))
                                    boxname += " Plus";
                        ...
                        var template = this.wtvshared.getTemplate(
                            "wtv-guide",
                            "templates/glossary_word_index.js",
                            true
                        );
                        var isPlusBox = false;
                        if (this.session_data.hasCap("client-has-tv-experience"))
                        ...
                        Object.keys(glossary[subtopic.toUpperCase()]).forEach(function (k) {
                            if (glossary[subtopic.toUpperCase()][k].plusonly && !isPlusBox)
                                return;
            ...
            default:
                ...
                try {
                    ...
                    if (!page)
                        prerendered = this.wtvshared.getTemplate(
                            "wtv-guide",
                            "prerendered/" + topic + "/" + subtopic + ".js",
                            true
                        );
                    else
                        prerendered = this.wtvshared.getTemplate(
                            "wtv-guide",
                            "prerendered/" + topic + "/" + subtopic + "/" + page + ".js",
                            true
                        );
