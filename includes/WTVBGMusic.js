class WTVBGMusic {
    ...
    getMusicObj(force_default = false, playlist) {
		var sessionstore_playlist = null;
		
		if (this.session_data.hasCap("client-can-do-rmf")) {
			sessionstore_playlist = "wtv-bgmusic-rmf"
		} else {
			sessionstore_playlist = "wtv-bgmusic-mid"
		}
		
		if (playlist) {
			sessionstore_playlist = playlist
		}

        var music_obj = this.session_data.getSessionData(sessionstore_playlist);
        ...
        if (setDefaults === true) {
            // set up defaults
            if (sessionstore_playlist == "wtv-bgmusic-rmf") {
                // rmf
                music_obj.enableCategories = [
                    "1",
                    "2",
                    "3",
                    "7",
            ...
            this.session_data.setSessionData(sessionstore_playlist, music_obj);
	...
	convertPlaylist() {
		console.log("* Legacy playlist detected, converting...")
		// grab legacy playlist from sessionstore
		var music_obj = this.session_data.getSessionData("wtv-bgmusic");
        if (music_obj === null) return true;
		
		var music_obj_rmf = null;
		var music_obj_mid = null;
		// migrate playlist to whichever playlist the currently connected box will be using
		if (this.session_data.hasCap("client-can-do-rmf")) { 
			music_obj_rmf = music_obj 
		} else { 
			music_obj_mid = music_obj 
		}
		// set defaults for uninitalized playlists
		if (music_obj_rmf == null) {
			this.getMusicObj(true, "wtv-bgmusic-rmf")
		}
		
		if (music_obj_mid == null) {
			this.getMusicObj(true, "wtv-bgmusic-mid")
		}
		
		if (music_obj_rmf != null) {
			this.session_data.setSessionData("wtv-bgmusic-rmf", music_obj_rmf);
		} else if (music_obj_mid != null) {
			this.session_data.setSessionData("wtv-bgmusic-mid", music_obj_mid);
		}
		this.session_data.deleteSessionData("wtv-bgmusic")
    ...
    getCategorySongList(category) {
        ...
        Object.keys(musiclist).forEach(function (k) {
            ...
            if (String(category).length === 1) {
                // 3 digit song id
                if (
                    parseInt(k.substr(0, 1)) == parseInt(category) &&
                    String(k).length === 3
                )
                    songList.push(musiclist[k]);
            } else if (String(category).length === 2) {
                // 4 digit song id
                if (
                    parseInt(k.substr(0, 2)) == parseInt(category) &&
                    String(k).length === 4
                )
                    songList.push(musiclist[k]);
            }
        ...
        Object.keys(self.categories).forEach(function (k) {
            ...
            if (songList.length > 0)
                enabledCategories.push({
                    id: parseInt(k) + 1,
                    name: self.categories[k],
                });
        });
        return enabledCategories.filter(
            (value) => Object.keys(value.name).length !== 0
        );
    }
