...
class WTVClientSessionData {
    ...
    clientAddress = null;
    user_id = 0;
    cryptoKey = "FHjDK8fd89sghdG89gfdhjskaGKFIOSP";

    constructor(wtvrsvc_config, ssid) {
        ...
        this.session_store = {};
        this.lockdownWhitelist = wtvrsvc_config.config.lockdownWhitelist;
        this.lockdownWhitelist.push(wtvrsvc_config.config.unauthorized_url);
        this.lockdownWhitelist.push(wtvrsvc_config.config.service_logo);
        this.mailstore = new WTVMail(this.wtvrsvc_config, this);
        this.favstore = new WTVFavorites(this.wtvrsvc_config, this);
        this.pagestore = new WTVAuthor(this.wtvrsvc_config, this);
        ...
        this.loginWhitelist.push("wtv-register:/*"); // we need register to be accessible for unregistered users, dumbass
        this.loginWhitelist.push("wtv-flashrom:/*"); // flashrom needs to be accessible for the bf0 undementiainator to do its thing
        this.loginWhitelist.push("wtv-star:/*"); // cry about it
    }
    ...
    assignFavoriteStore() {
        this.mailstore = this.favstore = new WTVFavorites(
            this.wtvrsvc_config,
            this
        );
    }
    ...
    createWTVSecSession() {
        return new WTVSec(this.wtvrsvc_config);
    }

    //this was really pissing me off so i inserted old code. fuck. i am at my god damned limit
    getAccountTotalUnreadMessages() {
        var total_unread_messages = 0;
        for (
            var i = 0;
            i < this.wtvrsvc_config.config.user_accounts.max_users_per_account;
            i++
        ) {
            var subUserSession = new this.constructor(this.wtvrsvc_config, this.ssid);
            subUserSession.switchUserID(i, false, false);
            subUserSession.assignMailStore();
            if (subUserSession.mailstore) {
                total_unread_messages +=
                    subUserSession.mailstore.countUnreadMessages(0);
            }
        }
    ...
    switchUserID(
        ...
        if (user_id != null) {
            ...
            if (this.data_store.wtvsec_login && update_ticket)
                this.setTicketData("user_id", user_id);
        } else {
            this.user_id = 0;
    ...
    setTicketData(key, value) {
        if (this.data_store.wtvsec_login)
            this.data_store.wtvsec_login.setTicketData(key, value);
        else return false;
    ...
    getTicketData(key) {
        if (this.data_store.wtvsec_login)
            return this.data_store.wtvsec_login.getTicketData(key);
    ...
    deleteTicketData(key) {
        if (this.data_store.wtvsec_login)
            this.data_store.wtvsec_login.deleteTicketData(key);
        else return false;
    ...
    findFreeUserSlot() {
        ...
        if (this.fs.existsSync(master_directory)) {
            for (
                ...
                // this was written at 1AM in a fit of rage
                // note to self: make koko fix this
                if (!this.fs.existsSync(test_dir)) {
                    return i;
                    break;
                } else {
                    var files = this.fs.readdirSync(test_dir);
                    var jasons = [];
                    files.forEach((file) => {
                        if (this.path.extname(file) == ".json") jasons.push(file);
                    });
                    console.log(jasons);
                    if (jasons.length == 0) {
                        this.fs.rmSync(test_dir, { recursive: true, force: true });
                        return i;
                        break;
                    }
                }
    ...
    getDisplayName() {
        return this.user_id == 0
            ? this.getSessionData("subscriber_name")
            : this.getSessionData("display_name");
    }
    ...
    listPrimaryAccountUsers() {
        // no longer subscriber only command if (this.user_id != 0) return false;
        ...
        this.fs.readdirSync(master_directory).forEach((f) => {
            if (
                self.fs.lstatSync(master_directory + self.path.sep + f).isDirectory()
            ) {
                if (f.substr(0, 4) == "user") {
                    var user_file =
                        master_directory + self.path.sep + f + self.path.sep + f + ".json";
                    if (self.fs.existsSync(user_file)) {
                        if (f == "user0") {
                            account_data["subscriber"] = JSON.parse(
                                this.fs.readFileSync(user_file)
                            );
    ...
    mkdirRecursive(thedir) {
        thedir.split(this.path.sep).reduce((directories, directory) => {
    ...
    getAccountStoreDirectory() {
        return this.wtvrsvc_config.config.SessionStore + this.path.sep + "accounts";
    }
    ...
    storeUserStoreFile(path, data, last_modified = null, overwrite = true) {
        ...
        var store_dir_path = this.wtvshared.makeSafePath(
            store_dir,
            path_split.join("/").replace("/", this.path.sep)
        );
        var store_full_path = this.wtvshared.makeSafePath(
            store_dir_path,
            file_name
        );

        try {
            if (!this.fs.existsSync(store_dir_path))
                this.fs.mkdirSync(store_dir_path, { recursive: true });
            var file_exists = this.fs.existsSync(store_full_path);
            if (!file_exists || (file_exists && overwrite))
                result = this.fs.writeFileSync(store_full_path, data);
    ...
    getUserStoreFile(path) {
        ...
        var store_dir_path = this.wtvshared.makeSafePath(
            store_dir,
            path.replace("/", this.path.sep)
        );
        if (this.fs.existsSync(store_dir_path))
            return this.fs.readFileSync(store_dir_path);
        else return false;
    }
    ...
    resetCookies() {
        ...
        this.addCookie(
            "wtv.zefie.com",
            "/",
            this.wtvshared.getUTCTime(365 * 86400000),
            "cookie_type=chocolatechip"
        );
    }
    ...
    addCookie(domain, path = null, expires = null, data = null) {
        ...
        if (!domain) return false;
            ...
            if (domain.domain && domain.path && domain.expires && domain.data)
                var cookie_data = domain;
            else return false;
        } else {
        ...
        Object.keys(this.session_store.cookies).forEach(function (k) {
            ...
            if (
                domain == self.session_store.cookies[k].domain &&
                path == self.session_store.cookies[k].path
            )
                cookie_index = k;
        });
    ...
    getCookie(domain, path) {
        ...
        Object.keys(this.session_store["cookies"]).forEach(function (k) {
            ...
            if (
                self.session_store["cookies"][k].domain == domain &&
                self.session_store["cookies"][k].path == path
            ) {
                var current_epoch_utc = Date.parse(new Date().toUTCString());
                var cookie_expires_epoch_utc = Date.parse(
                    new Date(
                        Date.parse(self.session_store["cookies"][k].expires)
                    ).toUTCString()
                );
                if (cookie_expires_epoch_utc <= current_epoch_utc)
                    self.deleteCookie(self.session_store["cookies"][k]);
    ...
    listCookies() {
        ...
        Object.keys(this.session_store.cookies).forEach(function (k) {
            outstring +=
                self.session_store.cookies[k].domain +
                "\0" +
                self.session_store.cookies[k].path +
                "\0";
        });
        return outstring;
    }
    ...
    loadSessionData(raw_data = false) {
        try {
            if (
                this.fs.lstatSync(
                    this.getUserStoreDirectory() + "user" + this.user_id + ".json"
                )
            ) {
                var json_data = this.fs.readFileSync(
                    this.getUserStoreDirectory() + "user" + this.user_id + ".json",
                    "Utf8"
                );
            ...
            if (e.code != "ENOENT")
                console.error(
                    " # Error loading session data for",
                    this.wtvshared.filterSSID(this.ssid),
                    e
                );
    ...
    decryptPassword(crypt) {
        return CryptoJS.AES.decrypt(crypt, this.cryptoKey).toString(
            CryptoJS.enc.Utf8
        );
    }
    ...
    getUserSMTPPassword() {
        return this.decryptPassword(
            this.setSessionData("subscriber_smtp_password")
        );
    }
    ...
    getUserPasswordEnabled() {
        if (!this.wtvrsvc_config.config.passwords.enabled) return false; // master config override
    ...
    saveSessionData(force_write = false, skip_merge = false) {
        if (this.isRegistered()) {
            if (!skip_merge) {
                ...
                if (temp_data)
                    this.session_store = Object.assign(temp_data, this.session_store);
                temp_data = null;
            }
        } else {
        ...
        try {
            ...
            if (json_save_data != json_load_data)
                this.fs.writeFileSync(
                    storeDir + "user" + this.user_id + ".json",
    ...
    isRegistered(session_mode = true) {
        if (session_mode)
            return (
                this.getSessionData("registered") &&
                this.fs.existsSync(this.getUserStoreDirectory())
            );
        else return this.fs.existsSync(this.getUserStoreDirectory());
    }

    unregisterBox() {
        var user_store_base = this.wtvshared.makeSafePath(
            this.wtvshared.getAbsolutePath(this.getAccountStoreDirectory()),
            this.path.sep + this.ssid
        );
        try {
            ...
            // Don't log error 'file not found', it just means the client isn't registered yet
            console.error(
                " # Error deleting session data for",
                this.wtvshared.filterSSID(this.ssid),
                e
            );
            return false;
    ...
    isMiniBrowser() {
        return this.data_store["wtv-need-upgrade"] ||
            this.data_store["wtv-used-8675309"]
            ? true
            : false;
    }
    ...
    checkSecurity() {
        ...
        var rejectSSIDConnection = function (blacklist) {
            if (blacklist) {
                ...
                console.log(
                    " * Request from SSID",
                    self.wtvshared.filterSSID(self.ssid),
                    "(" + self.clientAddress + "), but that SSID is in the blacklist."
                );
            } else {
                ...
                console.log(
                    " * Request from SSID",
                    self.wtvshared.filterSSID(self.ssid),
                    "(" + self.clientAddress + "), but that SSID is not in the whitelist."
                );
            }
        };
        ...
        var checkSSIDIPWhitelist = function (ssid, blacklist) {
            ...
            if (self.wtvrsvc_config.config.ssid_ip_allow_list) {
                if (self.wtvrsvc_config.config.ssid_ip_allow_list[self.ssid]) {
                    Object.keys(
                        self.wtvrsvc_config.config.ssid_ip_allow_list[self.ssid]
                    ).forEach(function (k) {
                        if (
                            self.wtvrsvc_config.config.ssid_ip_allow_list[self.ssid][
                                k
                            ...
                            if (
                                isInSubnet(
                                    self.clientAddress,
                                    self.wtvrsvc_config.config.ssid_ip_allow_list[self.ssid][k]
                                )
                            ) {
                    ...
                    if (!ssid_access_list_ip_override)
                        rejectSSIDConnection(self.ssid, blacklist);
                } else {
            ...
            if (
                ssid_access_list_ip_override &&
                self.wtvrsvc_config.config.debug_flags.debug
            )
                console.log(
                    " * Request from disallowed SSID",
                    wtvshared.filterSSID(ssid),
                    "was allowed due to IP address whitelist"
                );
        };

        // process whitelist first
        if (self.ssid && self.wtvrsvc_config.config.ssid_allow_list) {
            var ssid_is_in_whitelist =
                self.wtvrsvc_config.config.ssid_allow_list.findIndex(
        ...
        // now check blacklist
        if (self.ssid && self.wtvrsvc_config.config.ssid_block_list) {
            var ssid_is_in_blacklist =
                self.wtvrsvc_config.config.ssid_block_list.findIndex(
                    (element) => element == self.ssid
                );
		...
		if (self.clientAddress && self.wtvrsvc_config.config.ip_block_list) {
            var ip_is_in_blacklist =
                self.wtvrsvc_config.config.ip_block_list.findIndex(
                    (element) => element == self.clientAddress
                );
				if (ip_is_in_blacklist != -1) {
					rejectReason = "Bzzzzt! Thanks for playing!"
				}
        }
    ...
    isAuthorized(url, whitelist = "lockdown", ignore_lockdown = false) {
        ...
        if (whitelist == "lockdown" && !this.lockdown && !ignore_lockdown)
            return true;
        ...
        switch (whitelist) {
            case "lockdown":
                Object.keys(this.lockdownWhitelist).forEach(function (k) {
                    if (
                        self.lockdownWhitelist[k].charAt(
                            self.lockdownWhitelist[k].length - 1
                        ) == "*"
                    ) {
                        if (
                            self.lockdownWhitelist[k].substring(
                                0,
                                self.lockdownWhitelist[k].length - 1
                            ) == url.substring(0, self.lockdownWhitelist[k].length - 1)
                        )
                            authorized = true;
                    } else {
                        if (self.lockdownWhitelist[k].substring(0, url.length) == url)
                            authorized = true;
                    }
                });
                break;
            case "login":
                Object.keys(this.loginWhitelist).forEach(function (k) {
                    if (
                        self.loginWhitelist[k].charAt(self.loginWhitelist[k].length - 1) ==
                        "*"
                    ) {
                        if (
                            self.loginWhitelist[k].substring(
                                0,
                                self.loginWhitelist[k].length - 1
                            ) == url.substring(0, self.loginWhitelist[k].length - 1)
                        )
                            authorized = true;
                    } else {
                        if (self.loginWhitelist[k].substring(0, url.length) == url)
                            authorized = true;
                    }
                });
                break;
