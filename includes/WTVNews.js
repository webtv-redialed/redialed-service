class WTVNews {
                ...
                .then((response) => {
                    if (response.code == 200 || response.code == 201) {
                        ...
                        if (this.username && this.password) {
                            this.client
                                .authInfoUser(this.username)
                                .then((res) => {
                                    if (res.code == "381") {
                                        res
                                            .authInfoPass(this.password)
                                            .then((res) => {
    ...
    listGroup(group, page = 0, limit = 100, raw_range = null) {
        ...
        return new Promise((resolve, reject) => {
            this.selectGroup(group)
                .then((res) => {
                    if (!raw_range) {
						// this makes me want to krill myshelf
						var start;
						var end;
						
                        end = res.group.high - page * 100;
						start = end - 100;
        ...
        return new Promise((resolve, reject) => {
            this.client
                ...
                .catch((e) => {
                    console.error(" * WTVNews Error:", "Command: selectGroup", e);
                    reject(
                        `The news group <blackface>${group}</blackface> does not exist.`
                    );
                });
        });
    }
                ...
                .then((data) => {
                    if (get_next_last) {
                        ...
                        Promise.all(promises).then(() => {
                            var self = this;
                            if (data.article.headers)
                                Object.keys(data.article.headers).forEach((k) => {
                                    data.article.headers[k] = self.decodeCharset(
                                        data.article.headers[k]
                                    );
                                });
                            resolve(data);
                        });
    ...
    postToGroup(
        ...
        return new Promise((resolve, reject) => {
            var promises = [];
            var messageid = null;
            this.connectUsenet().then(() => {
                if (article) {
                    promises.push(
                        new Promise((resolve, reject) => {
                            this.selectGroup(group)
                                .then((res) => {
                                    this.getArticleMessageID(article)
                                        .then((data) => {
                                            messageid = data;
                                            resolve(data);
                                        })
                                ...
                                response
                                    ...
                                    .then((response) => {
                                        ...
                                        if (response.code !== 240) {
                                            reject(
                                                "Could not send post. Server returned error " +
                                                response.code
                                            );
                                        } else {
                                            resolve(true);
    ...
    getHeaderObj(NGArticles) {
        return new Promise((resolve, reject) => {
            var messages = [];
            var promises = [];
            for (let article = 0; article < NGArticles.length; article++) {
                if (article == "getCaseInsensitiveKey") continue;
				if (typeof NGArticles[article] != "number") break;
                promises.push(
                    new Promise((resolve, reject) => {
                        this.getHeader(NGArticles[article])
                            .then((data) => {
    ...
    parseAttachments(message) {
        ...
        if (contype) {
            ...
            if (match) {
                ...
                body.forEach((element) => {
                    ...
                    section.forEach((line) => {
                        ...
                        if (section_header_match) {
                            ...
                            if (section_match)
                                attachments[i].content_encoding = section_match[1];
                        } else {
                ...
                var message_body = "";
                if (message.article.body)
                    message_body = message.article.body.join("\n");
    ...
    sortByResponse(messages) {
        ...
        Object.keys(messages).forEach((k) => {
            ...
            if (ref) {
                ...
                if (res) {
                    ...
                    if (message_relations[res.messageId])
                        message_relations[res.messageId].push({
                            messageId: messageId,
                            index: k,
                        });
                    else
                        message_relations[res.messageId] = [
                            {messageId: messageId, index: k},
                        ];
                } else {
                            ...
                            if (message_relations[j].length > 0) {
                                Object.keys(message_relations[j]).forEach((h) => {
                                    ...
                                    if (message_relations[j][h].messageId == ref) {
                                        var searchref =
                                            messages[message_relations[j][h].index].headers
                                                .REFERENCES || null;
                                        var mainref = null;
                                        while (searchref !== null) {
                                            var searchart = messages.find(
                                                (e) => e.messageId == searchref
                                            );
                                            var searchref = searchart.headers.REFERENCES || null;
                                        }
                                        mainref = searchart.messageId;
                                        message_relations[mainref].push({
                                            messageId: messageId,
                                            index: k,
                                        });
                                        found = true;
        ...
        Object.keys(message_id_roots).forEach((k) => {
            ...
            message_roots_sorted.push({
                article: article,
                relation: null,
                date: article_date,
            });
        });
        message_roots_sorted.sort((a, b) => {
        ...
        Object.keys(message_roots_sorted).forEach((k) => {
            ...
            if (message_relations[message_id_roots[k].messageId]) {
                var relations = [];
                Object.keys(message_relations[message_id_roots[k].messageId]).forEach(
                    (j) => {
                        // sort relations by date
                        var article =
                            messages[
                                message_relations[message_id_roots[k].messageId][j].index
                                ];
                        var article_date = Date.parse(article.headers.DATE);
                        relations.push({
                            article: article,
                            relation: message_id_roots[k].messageId || null,
                            date: article_date,
                        });
                    }
                );
