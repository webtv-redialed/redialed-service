...
class WTVMime {
                ...
                if (ssid_session.capabilities["client-can-receive-compressed-data"]) {
                    ...
                    if (ssid_session) {
                        ...
                        if (
                            ...
                            if (ssid_session.get("wtv-client-rom-type"))
                                is_softmodem = ssid_session
                                    .get("wtv-client-rom-type")
                                    .match(/softmodem/);
                            if (
                                !is_bf0app &&
                                ((!is_softmodem && !isOldBuild) ||
                                    (is_softmodem && !isOldBuild))
                            ) {
                    ...
                    if (typeof headers_obj == "string") content_type = headers_obj;
                        ...
                        content_type =
                            typeof headers_obj["wtv-modern-content-type"] != "undefined"
                                ? headers_obj["wtv-modern-content-type"]
                    ...
                    if (content_type) {
                        ...
                        if (
                            ...
                            content_type != "text/tellyscript"
                        )
                            compress_data = true;
                        else if (content_type.match(/^application\/(x-?)javascript$/))
                            compress_data = true;
                        ...
                        if (compression_type == 2) {
                            ...
                            if (
                                content_type.match(
                                    /^audio\/(x-)?(s3m|mod|xm|midi|wav|wave|aif(f)?)$/
                                )
                            )
                                compress_data = true; // s3m, mod, xm, midi & wav
                            if (content_type.match(/^application\/karaoke$/))
                                compress_data = true; // midi karaoke
                            if (content_type.match(/^binary\/(x-wtv-approm|doom-data)/))
                                compress_data = true; // approms and DOOM WADs
                            if (content_type.match(/^wtv\/download-list$/))
                                compress_data = true; // WebTV Download List
                        }
                    }
                }
            }
    ...
    getContentType(path) {
        ...
        if (modern_mime_type === false)
            modern_mime_type = "application/octet-stream";
    ...
    generateMultipartMIME(tuples, options) {
        ...
        options = options || {};
        var preamble =
            options.preamble || "This is a multi-part message in MIME format.";
        var epilogue = options.epilogue;
        var boundary =
            options.boundary || "------------" + this.wtvshared.generateString(24);
        ...
        var encapsulations = tuples.map(function (tuple, i) {
            ...
            var headers = [
                `Content-Type: ${mimetype}; ${
                    use_base64
                        ? `name="${filename}"`
                        : `charset=${encoding.toUpperCase()}; format=flowed`
                }`,
            ];
            ...
            headers.push(
                `Content-Transfer-Encoding: ${use_base64 ? "base64" : "7bit"}`
            );

            var bodyPart = headers.join(CRLF) + CRLF + CRLF;
            if (use_base64 && !is_base64)
                bodyPart +=
                    wtvshared.lineWrap(
                        Buffer.from(tuple.content).toString("base64"),
                        72
                    ) + CRLF;
