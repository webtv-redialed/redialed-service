class WTVFavorites {
    ...
    checkFavIntroSeen() {
        return this.wtvclient.getSessionData("subscriber_fav_intro_seen")
            ? this.wtvclient.getSessionData("subscriber_fav_intro_seen")
            : false;
    }

    setFavIntroSeen(seen) {
        this.wtvclient.setSessionData(
            "subscriber_fav_intro_seen",
            seen ? true : false
        );
    }
    ...
    createDefaultFolders() {
        ...
        if (brandId == 7) this.createTemplateFolder("Personal (Samsung)");
        else this.createTemplateFolder("Personal");

        if (brandId == 0) this.createTemplateFolder("Sony");
    }

    createFavstore() {
        if (this.favstoreExists() === false) {
            if (!this.fs.existsSync(this.favstore_dir))
                this.fs.mkdirSync(this.favstore_dir, {recursive: true});
    ...
    createFavorite(title, url, folder, image, imagetype) {
        ...
        if (imagetype != "url") image = btoa(image);
        ...
        try {
            if (this.fs.existsSync(favoritefileout)) {
                console.log(
                    " * ERROR: Favorite with this UUID (" +
                    favoriteid +
                    ") already exists (should never happen). Favorite lost."
                );
                return false;
            }
    ...
    listFavorites(folder) {
        ...
        var self = this;
        self.messageArr = [];
        this.fs.readdirSync(folderpath).map(function (v) {
            var favorite_data_raw = null;
            var favoritepath = folderpath + self.path.sep + v;
            if (self.fs.existsSync(favoritepath))
                favorite_data_raw = self.fs.readFileSync(favoritepath);
    ...
    getFavorite(folder, favoriteid) {
        ...
        if (this.fs.existsSync(folder_file_in))
            folder_data_raw = this.fs.readFileSync(folder_file_in);
        ...
        if (folder_data_raw) {
            ...
            if (folder_data) {
                ...
                console.error(" # FavErr: could not parse json in ", folder_file_in);
        }
        return false;
    }
    ...
    updateFavorite(favoritedata, folder) {
        ...
        var result = this.fs.writeFileSync(
            folderpath + favoritedata.id + ".zfav",
