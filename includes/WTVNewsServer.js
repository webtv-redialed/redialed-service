class WTVNewsServer {
        ...
        if (run_server) {
            ...
            nntp_server.prototype = {
                ...
                _authenticate: function (session) {
                    // authenticate
                    if (
                        session.authinfo_user == self.username &&
                        session.authinfo_pass == self.password
                    ) {
                ...
                _postArticle: function (session) {
                    try {
                        session.group.name = self.getHeader(
                            session.post_data,
                            "newsgroups"
                        );
                ...
                _getLast: function (session) {
                    ...
                    if (!session.group.current_article)
                        return nntp_statuses._420_ARTICLE_NOT_SLCTD;
                    if (
                        !self.articleExists(
                            session.group.name,
                            session.group.current_article
                        )
                    )
                        return nntp_statuses._420_ARTICLE_NOT_SLCTD;
                    var res = self.getLastArticle(
                        session.group.name,
                        session.group.current_article
                    );
            ...
            var tls_options = {
                ca: this.fs.readFileSync(
                    tls_path + this.path.sep + "localserver_ca.pem"
                ),
                key: this.fs.readFileSync(
                    tls_path + this.path.sep + "localserver_key.pem"
                ),
                cert: this.fs.readFileSync(
                    tls_path + this.path.sep + "localserver_cert.pem"
                ),
            };
            this.local_server = new nntp_server({
                requireAuth: using_auth,
                tls: tls_options,
                secure: true,
                allow_posting: true,
            });
    ...
    refartServer() {
		console.log("pissing pants")
		this.local_server.close()
		this.local_server.listen("nntps://127.0.0.1:57319");
    }

    getHeader(message, header) {
        if (message.headers) {
            var search = Object.keys(message.headers).find(
                (e) => e.toLowerCase() == header.toLowerCase()
            );
    ...
    createDataStore() {
        if (!this.fs.existsSync(this.data_path))
            return this.fs.mkdirSync(this.data_path);
        return true;
    }
    ...
    postArticle(group, post_data) {
        ...
        try {
            ...
            post_data.headers.Date = this.strftime(
                "%a, %-d %b %Y %H:%M:%S %z",
                Date.parse(post_data.headers.date)
            );

            // server added Injection-Date
            post_data.headers["Injection-Date"] = this.strftime(
                "%a, %-d %b %Y %H:%M:%S %z",
                Date.parse(Date.now())
            );

            // Reorder headers per examples in RFC3977 sect 6.2.1.3, not sure if needed
            post_data.headers = this.wtvshared.moveObjectElement(
                "Path",
                null,
                post_data.headers,
                true
    ...
    getArticle(group, article) {
        ...
        try {
            ...
            if (!this.findHeaderCaseInsensitive(data.headers, "subject"))
    ...
    getGroups(wildmat = null) {
        ...
        this.fs.readdirSync(this.data_path).forEach((file) => {
            if (
                this.fs.lstatSync(this.data_path + this.path.sep + file).isDirectory()
            ) {
                if (wildmat) {
