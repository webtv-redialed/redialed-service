class WTVAdmin {
    ...
    rejectConnection(reason_is_ssid) {
        ...
        if (reason_is_ssid) {
            ...
            console.log(
                " * Request from SSID",
                this.wtvshared.filterSSID(this.wtvclient.ssid),
                "(" +
                this.clientAddress +
                ") for wtv-admin, but that SSID is not in the admin whitelist."
            );
        } else {
            rejectReason =
                this.clientAddress +
                " is not in the whitelist for SSID " +
                this.wtvclient.ssid +
                ".";
            console.log(
                " * Request from SSID",
                ...
                "(" +
                this.clientAddress +
                ") for wtv-admin, but that IP is not authorized for that SSID."
            );
        }
        return rejectReason;
    }
    ...
    isAuthorized() {
        ...
        return allowed_ssid && allowed_ip
            ? true
            : this.rejectConnection(!allowed_ssid);
    }
    ...
    getAccountInfo(username, directory = null) {
        ...
        this.fs.readdirSync(search_dir).forEach((file) => {
            if (
                self.fs.lstatSync(search_dir + self.path.sep + file).isDirectory() &&
                account_data === null
            ) {
                account_data = self.getAccountInfo(
                    username,
    ...
    getAccountInfoBySSID(ssid) {
        ...
        if (userSession.isRegistered(false)) {
            ...
            account_info.username =
                account_info.account_users["subscriber"].subscriber_username;
