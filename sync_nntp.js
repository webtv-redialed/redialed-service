// no wildcard (yet?)
const groups_to_sync = ["webtv.users"];
...
const serviceName = "wtv-news";

if (!minisrv_config.services[serviceName].upstream_address) {
...
const service_config = minisrv_config.services[serviceName];
        ...
        fs.readdirSync(g).forEach((file) => {
            ...
            if (!articles.find((e) => e == articleNumber)) {
                console.log(
                    ...
                    "article",
                    articleNumber,
                    "deleted from upstream, removing locally"
                );
                fs.rmSync(file);
            }
        });
