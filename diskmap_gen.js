...
if (process.argv.length < 6) {
    ...
    console.error(
        ...
        process.argv[0],
        process.argv[1],
        "content/Demo/ Demo.json DealerDemo file://Disk/Demo/"
    );
    process.exit(1);
}
...
if (!service_vault) {
    console.error(
        "Could not find",
        service_vault_subdir,
        "in any configured Service Vaults!"
    );
    process.exit(1);
}
...
if (fileList.length > 0) {
    var diskmap = {};
    diskmap[group_name] = {};
    if (client_dest.substring(client_dest.length - 1, 1) != "/")
        client_dest += "/";
    ...
    fileList.forEach(function (v) {
        diskmap[group_name].files.push({
            file: v
                .replace(service_vault_dir, client_dest)
