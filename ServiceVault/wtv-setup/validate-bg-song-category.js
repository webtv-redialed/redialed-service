...
if (session_data.hasCap("client-can-do-rmf")) {
	playlist = "wtv-bgmusic-rmf"
} else {
	playlist = "wtv-bgmusic-mid"
}

if (request_headers.query && session_data) {
    if (request_headers.request_url.indexOf("?") >= 0) {
        var category = request_headers.query.category
        ...
        if (category == null) music_obj.enableCategories = [];
            ...
            if (cat) {
                ...
                Object.keys(music_obj.enableSongs).forEach(function (k) {
                    if (
                        wtvbgm.getSongCategory(parseInt(music_obj.enableSongs[k])) ==
                        parseInt(category)
                    )
                        toRemove.push(k);
                });
                ...
                var newEnableSongs = music_obj.enableSongs.filter(
        ...
        if (qraw.length > 0) {
            ...
            for (let i = 0; i < qraw.length; i++) {
                ...
                if (qraw_split.length == 2) {
                    var k = qraw_split[0];
                    if (k == "enableCategory")
                        music_obj["enableCategories"].push(
                            unescape(qraw[i].split("=")[1].replace(/\+/g, "%20"))
                        );
                    if (k == "enableSong")
                        music_obj["enableSongs"].push(
                            unescape(qraw[i].split("=")[1].replace(/\+/g, "%20"))
                        );
                }
            }
    ...
    music_obj.enableCategories = [
        ...new Set(
            music_obj.enableCategories.filter(
                (value) => Object.keys(value).length !== 0
            )
        ),
    ];
    music_obj.enableSongs = [
        ...new Set(
            music_obj.enableSongs.filter((value) => Object.keys(value).length !== 0)
        ),
    ];
    music_obj = Object.assign({}, music_obj);
    if (
        Object.keys(music_obj.enableCategories).length !=
        Object.keys(old_music_obj.enableCategories).length ||
        Object.keys(music_obj.enableSongs).length !=
        Object.keys(old_music_obj.enableSongs).length
    ) {
        // something changed
		
        session_data.setSessionData(playlist, music_obj);
