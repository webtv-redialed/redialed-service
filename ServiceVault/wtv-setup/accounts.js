...
if (session_data.user_id != 0) {
    var errpage = doErrorPage(
        400,
        "You are not authorized to edit the primary account."
    );
    headers = errpage[0];
    data = errpage[1];
} else {
...
<A HREF="wtv-setup:/edit-user-begin?user_id=${key.replace("user", "")}">${
                value.subscriber_username
            }</A>
</B></FONT><BR>
...
name="RemoveUser" value="Remove User" WIDTH=140 ${
        num_accounts >= 1 ? "USESTYLE" : 'disabled="disabled" text=gray'
    }
