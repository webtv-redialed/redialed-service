...
if (Object.keys(session_data.listPrimaryAccountUsers()).length == 1) {
    ...
    errpage = wtvshared.doErrorPage(
        400,
        "You are not authorized to remove users from this account."
    );
...
if (usersToRemove.length === 0)
    errpage = wtvshared.doErrorPage(400, "No users were specified for removal.");
...
if (errpage) {
    ...
    if (!request_headers.query.confirm_remove) {
        ...
        if (usersToRemove.length == 1) {
            ...
            message =
                "Removing the selected users will permanently remove their e-mail and favorites as well. You will not be able to restore the users.";
        }
