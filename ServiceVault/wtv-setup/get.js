...
Content-Type: text/html`;
    ...
    // defaults
    if (!settings_obj["setup-advanced-options"])
    ...
    if (!settings_obj["setup-visited-color"])
        settings_obj["setup-visited-color"] = "8822bb";
    if (!settings_obj["setup-japan-keyboard"])
        settings_obj["setup-japan-keyboard"] = "roman";
    if (!settings_obj["setup-japan-softkeyboard"])
        settings_obj["setup-japan-softkeyboard"] = "roman";
    if (!settings_obj["setup-chat-access-level"])
        settings_obj["setup-chat-access-level"] = 0;
    if (!settings_obj["setup-chat-on-nontrusted-pages"])
        settings_obj["setup-chat-on-nontrusted-pages"] = 1;
    if (!settings_obj["setup-tv-chat-level"])
