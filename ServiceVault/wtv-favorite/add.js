            ...
            const request = proxy_agent.get(url, options, (response) => {
                ...
                if (response.statusCode == 301 || response.statusCode == 302) {
                    ...
                    if (redirects < max_redirects)
                        resolve(getTitle(response.headers.location));
                    else
                        reject(
                            `Too many redirects. Max: ${max_redirects}, Current: ${redirects}`
                        );
                }
    ...
    if (favstore.favstoreExists()) {
        ...
        if (!title) {
            try {
                ...
                console.error(
                    " * Error: Client sent favorite-url without title, and we could not get the title from the server:",
                    e
                );
            }
        }
...
var url = request_headers.query;
var title = url["favorite-title"];
var folder = url["favorite-category"];
folder = folder.replaceAll("+", " ");
var imagetype = url["favorite-thumbnail-type"];
var favurl = url["favorite-url"];
