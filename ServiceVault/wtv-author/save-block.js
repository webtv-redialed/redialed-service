    ...
    case "23":
        if (editing == "true") {
            ...
            if (request_headers.query.toSnapshot == "true")
                session_data.pagestore.editPhotoBlock(
                    docName,
                    request_headers.query.blockNum,
                    request_headers.query.newBlockNum,
                    photo,
                    "snapshot",
                    request_headers.query.blockTitle,
                    request_headers.query.photoBlockCaption
                );
            else
                session_data.pagestore.editPhotoBlock(
                    docName,
...
Location: ${
                request_headers.query.returnPageURL ||
                "wtv-author:/show-blocks?docName=" + docName
            }`;
        } else {
            session_data.pagestore.createPhotoBlock(
                docName,
                request_headers.query.photoBlockPhoto,
                "snapshot",
                position
            );
            var page = session_data.pagestore.loadPage(docName);
...
Location: ${
    ...
    case "21":
        if (caption.length == 0) {
            headers =
                "400 You must enter a caption. Please enter a caption and try again.";
        } else {
            if (editing == "true") {
                if (size.length == 0) size = null;

                if (style.length == 0) style = null;
                session_data.pagestore.editTextBlock(
                    docName,
                    title,
                    caption,
                    size,
...
Location: wtv-author:/show-blocks?docName=${docName}`;
                ...
                if (style.length == 0) style = null;
                session_data.pagestore.createTextBlock(
                    docName,
                    title,
                    caption,
                    size,
...
Location: wtv-author:/show-blocks?docName=${docName}`;
    ...
    case "26":
        ...
        if (header.length == 0) {
            headers =
                "400 You must enter a header. Please enter a header and try again.";
        } else {
            if (editing == "true") {
                session_data.pagestore.editHeaderBlock(
                    docName,
                    header,
                    size,
                    dividerBefore,
...
Location: wtv-author:/show-blocks?docName=${docName}`;
                ...
                session_data.pagestore.createHeaderBlock(
                    docName,
                    header,
                    size,
                    dividerBefore,
...
Location: wtv-author:/show-blocks?docName=${docName}`;
    ...
    case "24":
        if (editing == "true") {
            var listItems = request_headers.query.listItemText.filter(function (e) {
                return e.replace(/(\r\n|\n|\r)/gm, "");
            });
            session_data.pagestore.editListBlock(
                docName,
                title,
                listItems,
                position,
...
Location: wtv-author:/show-blocks?docName=${docName}`;
            ...
            session_data.pagestore.createListBlock(
                docName,
                title,
                listItems,
                position
...
Location: wtv-author:/show-blocks?docName=${docName}`;
            ...
            var listItems = request_headers.query.listItemText;
            var linkItems = request_headers.query.linkItemURL;
            session_data.pagestore.createLinkBlock(
                docName,
                title,
                listItems,
                linkItems,
...
Location: wtv-author:/show-blocks?docName=${docName}`;
    ...
    case "27":
        if (editing == "true") {
            session_data.pagestore.editBreakBlock(
                docName,
                position,
                request_headers.query.blockNum
            );
