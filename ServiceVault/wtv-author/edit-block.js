...
var photo;
var thumbnail;
if (blockType == "clipart") {
    photo = atob(pagedata.blocks[oldBlockNum].photo);
...
var gInitialValue = "${wtvshared.htmlEntitize(pagedata.blocks[oldBlockNum].caption)}";
...
<tr>
<td absheight=28>
<input type=text name="blockTitle" value="${wtvshared.htmlEntitize(pagedata.blocks[oldBlockNum].title)}" width=100% text=CACA4A bgcolor=192133 font=proportional autoactivate>
<tr>
<td absheight=28 valign=bottom>
...
<TD colspan=3><INPUT type=checkbox name=headingBlockDividerBefore `;
        if (pagedata.blocks[oldBlockNum].dividerBefore == "on") data += "checked";
        data += `>
...
<TD colspan=3><INPUT type=checkbox name=headingBlockDividerAfter `;
        if (pagedata.blocks[oldBlockNum].dividerAfter == "on") data += "checked";
        data += `>
